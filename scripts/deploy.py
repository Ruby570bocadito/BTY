...
def banner():
    print(f"""{BOLD}{CYAN}
   ╔══════════════════════════════════════════════╗
   ║         {BOLD}BTY C2 Framework — Auto Deploy                          {RESET}{CYAN}       ║
...
def build_server():
    ...
    output = Path(__file__).parent.parent / "bty-server"
    ...
    if not go_bin:
        ...
        for p in [Path("bty-server"), Path("bin/ctrlboty-server"), Path("bin/byob-server")]:
            if p.exists():
                return p
        return None
...
def main():
    ...
    parser = argparse.ArgumentParser(description="BTY C2 Auto-Deploy")
    ...
    # Start server
    print(f"{BOLD}{GREEN}[>] Starting BTY C2 Server...{RESET}")
