#!/bin/bash
# BTY C2 - Automated Installation Script
...
echo -e "${BOLD}${CYAN}"
echo "   ╔══════════════════════════════════════════════╗"
echo "   ║         BTY C2 - Automated Installer         ║"
echo "   ╚══════════════════════════════════════════════╝"
echo -e "${RESET}"
...
# === Phase 2: Compile ===
echo -e "\n${BOLD}[Phase 2] Compiling BTY C2${RESET}"
...
if command -v go &> /dev/null; then
    ...
    echo -e "  ${GREEN}[✓]${RESET} Server compiled: $(ls -lh ../../bty-server | awk '{print $5}')"

    echo -e "  ${BLUE}[>]${RESET} Building agent..."
    CGO_ENABLED=0 go build -ldflags="-s -w" -o ../../bty-agent ./cmd/agent/main.go
    echo -e "  ${GREEN}[✓]${RESET} Agent compiled: $(ls -lh ../../bty-agent | awk '{print $5}')"
    cd ../..
else
...
if command -v openssl &> /dev/null; then
    ...
    openssl req -x509 -newkey rsa:4096 -keyout certs/server.key -out certs/server.crt \
        -days 365 -nodes -subj "/CN=localhost/O=BTY C2" \
...
# Check binaries
if [ -f "bty-server" ]; then
    echo -e "  ${GREEN}[✓]${RESET} bty-server: $(ls -lh bty-server | awk '{print $5}')"
else
    echo -e "  ${YELLOW}[!]${RESET} bty-server not found"
fi

if [ -f "bty-agent" ]; then
    echo -e "  ${GREEN}[✓]${RESET} bty-agent: $(ls -lh bty-agent | awk '{print $5}')"
else
    echo -e "  ${YELLOW}[!]${RESET} bty-agent not found"
fi
