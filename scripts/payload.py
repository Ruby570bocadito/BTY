...
def generate_python(server, name=None):
    out = OUTPUT_DIR/(name or f"bty-{datetime.now():%Y%m%d_%H%M}.py")
...
def generate_c(server, name=None):
    out = OUTPUT_DIR/(name or f"bty-{datetime.now():%Y%m%d_%H%M}.c")
...
def generate_evasive(server):
    ...
    payload=None
    for f in sorted(OUTPUT_DIR.glob("bty-linux-*"),key=lambda x:x.stat().st_mtime,reverse=True):
...
def main():
    p=argparse.ArgumentParser(description="BTY Payload Generator")
