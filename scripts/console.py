#!/usr/bin/env python3
"""
BTY Interactive Console — CLI shell for C2 operations.
...
class BTYConsole(cmd.Cmd):
    intro = f"""{BOLD}{CYAN}
   ╔══════════════════════════════════════════════╗
   ║           BTY Interactive Console            ║
   ║           ruby570bocadito                     ║
   ╚══════════════════════════════════════════════╝
{RESET}
Type {GREEN}help{RESET} to see available commands.
"""
    prompt = f"{BOLD}{GREEN}bty{RESET} > "
    ...
    def do_background(self, arg):
        ...
        self.current_session = None
        self.prompt = f"{BOLD}{GREEN}bty{RESET} > "
...
def main():
    p = argparse.ArgumentParser(description="BTY Interactive Console")
    ...
    console = BTYConsole(args.server, args.user, password)
