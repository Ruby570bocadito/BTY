...
build-server:
	...
	cd src/go && CGO_ENABLED=0 go build -ldflags="-s -w" -o ../../bty-server ./cmd/server/main.go

build-agent:
	@echo "Building agent..."
	cd src/go && CGO_ENABLED=0 go build -ldflags="-s -w" -o ../../bty-agent ./cmd/agent/main.go

# Cross-compile agents for all platforms
build-all:
	@echo "Building agents for all platforms..."
	cd src/go && \
		CGO_ENABLED=0 GOOS=linux GOARCH=amd64 go build -ldflags="-s -w" -o ../../dist/bty-agent-linux ./cmd/agent/main.go && \
		CGO_ENABLED=0 GOOS=windows GOARCH=amd64 go build -ldflags="-s -w" -o ../../dist/bty-agent-windows.exe ./cmd/agent/main.go && \
		CGO_ENABLED=0 GOOS=darwin GOARCH=amd64 go build -ldflags="-s -w" -o ../../dist/bty-agent-darwin-amd64 ./cmd/agent/main.go && \
		CGO_ENABLED=0 GOOS=darwin GOARCH=arm64 go build -ldflags="-s -w" -o ../../dist/bty-agent-darwin-arm64 ./cmd/agent/main.go
