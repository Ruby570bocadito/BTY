  ...
  methods: {
    ...
    async addOperator() {
      ...
      try {
        ...
        if (r.ok) {
          ...
          this.loadOperators()
          if (window.BTY?.notify) window.BTY.notify.success('Operator added')
        } else {
          if (window.BTY?.notify) window.BTY.notify.error('Failed to add operator')
        }
      } catch (e) {
        if (window.BTY?.notify) window.BTY.notify.error('Network error')
      }
      this.loading = false
    },
    async deleteOperator(id) {
      ...
      try {
        ...
        if (r.ok) {
          this.loadOperators()
          if (window.BTY?.notify) window.BTY.notify.success('Operator deleted')
        }
      } catch (e) {
        if (window.BTY?.notify) window.BTY.notify.error('Failed to delete')
      }
    },
    formatDate(dateStr) {
