#!/usr/bin/env python3
"""
BTY C2 - End-to-End Testing Framework
...
class E2ETestRunner:
    ...
    def start_server(self, tls=False):
        ...
        if os.path.exists("bty.db"):
            os.remove("bty.db")
        args = ["./bty-server", "-config", "config.yaml"]
        if not tls:
            args.append("-no-tls")
    ...
    def test_e2e_command_execution(self):
        ...
        # Test echo
        s, r = self.send_command(agent_id, "echo 'BTY E2E Test OK'")
        self.result("Command: echo", s == 200 and r.get('success', False) and 'BTY E2E Test OK' in r.get('output', ''),
    ...
    def run_all(self):
        print(f"\n{BOLD}{CYAN}{'='*60}{RESET}")
        print(f"{BOLD}{CYAN}  BTY C2 - End-to-End Testing{RESET}")
