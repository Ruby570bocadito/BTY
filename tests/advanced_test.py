#!/usr/bin/env python3
"""
BTY C2 - Advanced Test Framework
...
class TestRunner:
    ...
    def start_server(self, keep_db=False):
        ...
        if not keep_db and os.path.exists("bty.db"):
            os.remove("bty.db")
        self.server_proc = subprocess.Popen(
            ["./bty-server", "-config", "config.yaml", "-no-tls"],
    ...
    def test_docker_build(self):
        ...
        os.chdir("/mnt/c/Users/Rby/Desktop/BTY-master/BTY-master")
        r = self.docker("build -t bty-server -f Dockerfile . 2>&1 | tail -5", timeout=300)
    ...
    def test_docker_network(self):
        ...
        # Create network
        r = self.docker("network create bty-test-net 2>/dev/null; echo ok")
        self.result("Docker network create", "ok" in r.stdout)

        # Check Dockerfile exists and is valid
        dockerfile = Path("/mnt/c/Users/Rby/Desktop/BTY-master/BTY-master/Dockerfile")
        self.result("Dockerfile exists", dockerfile.exists())

        # Verify docker-compose.yml
        compose = Path("/mnt/c/Users/Rby/Desktop/BTY-master/BTY-master/docker-compose.yml")
    ...
    def run_all(self):
        print(f"\n{BOLD}{CYAN}{'='*60}{RESET}")
        print(f"{BOLD}{CYAN}  BTY C2 - Advanced Test Framework{RESET}")
